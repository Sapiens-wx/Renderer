...
void Mesh::LoadFromFile_Obj(std::ifstream& file) {
	...
	while (std::getline(file, line)) {
		...
		if (line.size()&&line[0] == 'v') {
			...
			if (sscanf_s(line.c_str() + 1, "%d/%d/%d %d/%d/%d %d/%d/%d", &v1, &t1, &n1, &v2, &t2, &n2, &v3, &t3, &n3) != 9) {
				CERR << "cannot parse the obj file. Currently does not support face lines other than 'f x/x/x x/x/x x/x/x'\n";
				return;
			}
			v1 -= 1; v2 -= 1; v3 -= 1; t1 -= 1; t2 -= 1; t3 -= 1; n1 -= 1; n2 -= 1; n3 -= 1; //deal with the fucking index starting from 1
			tris.push_back(v1);
			tris.push_back(v2);
			tris.push_back(v3);
